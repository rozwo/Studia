...
(define-type Exp
  ...
  (lamE [x : Symbol] [e : Exp])
  (appE [e1 : Exp] [e2 : Exp])
  (recE [f : Symbol] [arg : Symbol] [e : Exp]) ;dodanie rekurencji (do składni)
  )
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{rec SYMBOL {SYMBOL} ANY} s) ;parsowanie rekurencji
     (recE
...
(define-type Value
  ...
  (recV [f : Symbol] [x : Symbol] [e : Exp] [env : Env]) ;dodaje rekurencje
  )
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(recE f x e) ;ewaluacja rekurencji ogranicza się do stworzenie wartości rekurencyjnej
     (recV f x e env)]))
...
(define (apply [v1 : Value] [v2 : Value]) : Value
  (type-case Value v1
    ...
    [(primopV f)
     (f v2)]
    [(recV f x e env)                            ;w aplikacja dla rekurencji ewaluujemy wyrażenie e ze środowiskiem
     (eval e (extend-env (extend-env env x v2)   ;dla funkcji i jej wywołania rekurencyjnego, dla zmiennej 
                         f (recV f x e env)))]   ;której środowisko zostało powiększone o środowisko v2
    [else (error 'apply "not a function")]))

(define (run [e : S-Exp]) : Value
  (eval (parse e) init-env))

;dlaczego letrec (w ogolnej postaci) nie mógłby być potraktowany analogicznie?
;moze tam zostac wpisana zmienna a nie funkcja
;-> nie mozna tego traktowac jako funkcji i nie bedzie apply,
;-> rekurencja zwyczajnie nie zadziala
