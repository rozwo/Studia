...
(define (decompile-helper heap exp)
  (cond 
    [(empty? exp) (first heap)]
    [(pushI? (first exp)) 
      (decompile-helper 
        (cons (number->s-exp (pushI-n (first exp))) heap)
        (rest exp))]
    [(opI? (first exp))
      (decompile-helper
        (cons (list->s-exp (list (match-opI (first exp)) (second heap) (first heap))) (rest (rest heap)))
        (rest exp))]))

(define (decompile [c : Code])
  (decompile-helper (list) c))


(module+ test
  (test (decompile (compile (parse `2)))
        `2)
  (test (decompile (compile (parse `(/ 8 2))))
        `(/ 8 2))
  (test (decompile (compile (parse `(+ (* 2 3) (+ 5 8)))))
        `(+ (* 2 3) (+ 5 8)))
  (test (decompile (compile (parse `(+ (* 2 (/ 7 3)) (+ 5 (- 9 6))))))
        `(+ (* 2 (/ 7 3)) (+ 5 (- 9 6))))
  (test (decompile (list (pushI 2) (pushI 3) (opI (mul)) (pushI 5) (pushI 8) (opI (add)) (opI (add))))
        `(+ (* 2 3) (+ 5 8))))
