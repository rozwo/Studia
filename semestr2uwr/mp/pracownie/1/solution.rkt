...
#;(define cities2
  (table
   (list (column-info 'city    'string)
         ...
         (column-info 'area    'number)
         (column-info 'capital 'boolean))
   (list (list "Wrocław" "Poland"  693 #f)
         (list "Wrocław" "Poland"  193 #f)
         (list "Wrocław" "Wrocław"  293 #f)
         (list "Warsaw" "Warsaw"  293 #f)
         (list "Warsaw"  "Poland"  517 #t)
         (list "Poznań"  "Poland"  262 #f)
         (list "Berlin"  "Germany" 892 #t)
         (list "Munich"  "Germany" 310 #f)
...
(define (line s acc)
  (if (null? s)
      acc
      (line (cdr s) (append acc (list (column-info-name (car s)))))))

(define (good-line s c acc)
  (define (it v t)
    (cond [(null? t) #f]
          [(equal? (car t) v) #t]
          [else (it v (cdr t))]))
  (cond [(null? s) acc]
        [(it (car s) c) (good-line (cdr s) c (append acc (list (car s))))]
        [else (good-line (cdr s) c acc)]))

(define (good schem c)
  (good-line (line schem '()) c '()))

(define (table-project cols tab)

  (define colss (good (table-schema tab) cols)) ;gdyby cols sa nie po kolei to robie colss
                                                         ;i wtedy pomijam kolumny ktorych nazwy sa podane w cols
                                                                                       ;ale nie ma ich w tablicy
  ...
  (define (it-r t) ;bo potrzebuje to zrobic dla wszystkich wierszy
    (cond [(null? t) null]
          [(null? (cdr t)) (cons (it colss (table-schema tab) '() (car t)) null)]
          [else (cons (it colss (table-schema tab) '() (car t)) (it-r (cdr t)))]))
  
  (table (it colss (table-schema tab) '() (table-schema tab))
         (it-r (table-rows tab)))
  )
...
(define (table-select form tab)
  (define (it t acc)
    (cond [(null? t) acc]
          [(spelnia form (car t) (table-schema tab)) (it (cdr t) (append acc (list (car t))))]
          [else (it (cdr t) acc)]))
  (table (table-schema tab) (it (table-rows tab) '()))
  )

;test
;(table-select (eq2-f 'city 'country) cities2)
;(table-select (and-f (or-f (not-f (lt-f 'area 600)) (eq2-f 'city 'country)) (or-f (eq-f 'city "Wrocław") (eq-f 'city "Berlin"))) cities2)
   #;(list (list "Wrocław" "Poland"  693 #f)
         (list "Wrocław" "Wrocław"  293 #f)
         (list "Berlin"  "Germany" 892 #t))
...
(define (table-rename col ncol tab)
  (define (it s acc)
    (if (equal? (column-info-name (car s)) col)
        ...
        (it (cdr s) (append acc (list (car s))))))
  (if (null? tab)
      tab
...
(define (inne l s acc all-cols)
  (cond [(null? s) acc]
        [(null? l) (inne all-cols s acc all-cols)]
        [(is-in (car l) s) (inne (cdr l) (cdr s) acc all-cols)]
        [else (inne (cdr l) (cdr s) (append acc (list (car s))) all-cols)]))
...
(define (get-names schema result)
  (if (empty? schema)
      result
      (get-names (cdr schema) (append result (list (column-info-name (car schema)))))))

(define (table-natural-join tab1 tab2)
  (define l (same (table-schema tab1) (table-schema tab2)))
  (table-project (append (get-names (table-schema tab1) '()) (get-names (inne l (table-schema tab2) '() l) '()))
                 (t-s l (table-cross-join tab1 (t-r l tab2))))
  ;'() ;cos sie robi za dlugo wiec sprawdze czy to moze nie ta funkcja
  )
