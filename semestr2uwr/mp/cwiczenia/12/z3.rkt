...
(define-type Value
  (numV [n : Number])
  (boolV [b : Boolean])
  (funV [x : Symbol] [e : Exp])  ;bez srodowiska
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE x b)
     (funV x b)] ;dynamicznie -> bez srodowiska
    [(appE e1 e2)
     (apply (eval e1 env) (eval e2 env) env)])) ;dodajemy środowisko

;do apply przekażemy środowisko (nie przez funkcję)
